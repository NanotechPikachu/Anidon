...
function Anime() {
  ...
  return (
    ...
    {loading ? (
        <div className="flex flex-grow w-full min-h-screen items-center justify-center text-center text-lg md:text-xl text-violet-600"><p>Loading...</p></div>
      ) : m.title ? (
    <div>
    <div className="mt-20">
...
<Image src={m.image} />
    ...
    <div className="mt-2 flex flex-col md:grid md:grid-flow-row md:grid-cols-3 overflow-hidden md:gap-4"> {l?.map((x, index) => (<div key={index} onClick={() => changePage(n[index], m.title, index + 1)}><p className="border-2 border-violet-700 p-2 cursor-pointer">Episode: {x} </p></div>)) || 'No information'}</div>
    </div>
    </div>
...
export default function Info() {
  return (
    <Suspense fallback={<div className="flex flex-grow w-full min-h-screen items-center justify-center text-center text-lg md:text-xl dark:text-violet-600"><p>Loading...</p></div>}>
      <Anime />
    </Suspense>
  );
}