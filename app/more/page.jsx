...
async function More() {
  ...
  return (
    ...
    <img src={TRENDINGANIME} alt="Trending Anime" className="h-full w-1/2 object-contain" />
    </div>
    ...
    <img src={x.image} alt="Anime pic" className="w-full h-full object-over opacity-75 border-2 border-violet-500" />
    </div>
    ))}
    </div>
    </div>
    <div className="mt-16 w-full flex md:h-16 h-12 justify-center items-center">
    <img src={POPULARANIME2} alt="Popular Anime" className="h-full w-1/2 object-contain" />
    </div>
    <div className="hover:small-scroll mt-6 mb-4 grid grid-flow-col auto-cols-max max-w-screen grid-rows-1 gap-3 md:gap-6 overflow-x-auto h-36 md:h-56">
    {n.map((x, index) => (
    <div className="relative w-24 md:w-36 h-full flex items-stretch" key={x?.title?.romaji}>
