...
export async function GET(request) {
    ...
    const data = await fetch(`${animepaheUrl}/api?m=search&q=${encodeURIComponent(anime)}`, {
        headers: headers
    });
    let dataSheet;
    if (data?.status === 200) {
    ...
    if (dataSheet?.total === 0) return NextResponse.json({ message: "Anime Not Found" }, { status: 404 });
